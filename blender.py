...
def process_line(line):
    ...
    if fields[0] == '#':
        ...
        if len(fields) != 3 and fields[3] == 0.0000:
            if len(fields) == 4:
                print(fields[3])
...
def load_obj_file(file_name):
    ...
    return create_meshes_from_blender(vlist, flist, mlist, tlist, library,
                                      mesh_list, lnlist)


def create_meshes_from_blender(vlist, flist, mlist, tlist, library, mesh_list,
                               lnlist):
    fstart = 0
    mesh_id = 1
    meshes = []
    ...
    for f in range(len(flist)):
        if mesh_id != mesh_list[f]:  # new mesh is denoted by change in material
            ...
            try:
                mesh = create_mesh(varray, tarray, flist, fstart, f, library,
                                   material)
                meshes.append(mesh)
    ...
    try:
        mesh = create_mesh(
            varray, tarray, flist, fstart, len(flist), library, material)
        meshes.append(mesh)
    except:
        print('(W) could not load mesh!')
        raise
